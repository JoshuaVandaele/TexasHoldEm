...
def poker_table(screen: pygame.surface.Surface, players_list: list[str], bankroll: int = 1000, big_blind: int = 100) -> None:
    ...
    background: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/backgound_poker.jpeg"))

    # setup the dealer class
    dealer = Dealer([Player(player, Hand([Card(-1, 'init'), Card(-1, 'init')]), bankroll)
                    for player in players_list], big_blind)
    dealer.distribute(shuffle=True)

    # setup the board card and player list
    list_board_card = [pygame.image.load(resource_path(
        "jeu/assets/images/card/card_back.png"))] * 5
    list_players = [player for player in dealer.players]
    list_players_font = [menu_font.get_font(50).render(
        player, True, "#EEEEEE") for player in players_list]
    ...
    # setup the chip png
    board_chip: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/chip.png"))
    rect_board_chip: pygame.rect.Rect = board_chip.get_rect(
        center=(1280 / 2 + 350, 720 / 2))

    # setup the text for the current blind
    text_current_blind: pygame.surface.Surface = menu_font.get_font(
        25).render('Current Blind:', True, "#EEEEEE")
    rect_current_blind: pygame.rect.Rect = text_current_blind.get_rect(
        center=(1280 / 2 + 350, 720 / 2 + 75))

    # <----- PLAYERS TOKEN RECT LIST ----->

    # setup the list of tuple[surface, rect]
    players_token_rect: list[tuple[pygame.surface.Surface,
                                   pygame.rect.Rect]] = []
    ...
    rect_j1: pygame.rect.Rect = text_j1.get_rect(
        center=(1280 - 1105, 720 - 45))

    # setup the current player hand
    rect_card_j1_1: pygame.rect.Rect = list_players_hand[current_player][0].get_rect(
        center=(1280 / 2 - 75, 720 - 65))
    rect_card_j1_2: pygame.rect.Rect = list_players_hand[current_player][1].get_rect(
        center=(1280 / 2 + 75, 720 - 65))

    # setup the current player dealer token
    dealer_token_j1: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/Dealer_Token.png"))
    rect_j1_dealer: pygame.rect.Rect = dealer_token_j1.get_rect(
        center=(1280 / 2 - 225, 720 - 75))

    # setup the current player bankroll
    bankroll_j1: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/chip.png"))
    rect_bankroll_j1: pygame.rect.Rect = bankroll_j1.get_rect(
        center=(1280 / 2 + 225, 720 - 75))
    ...
    rect_j2: pygame.rect.Rect = text_j2.get_rect(
        center=(1280 - 175, 720 - 675))

    # setup the next player hand
    card_j2_1: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/card/card_back.png"))
    rect_card_j2_1: pygame.rect.Rect = card_j2_1.get_rect(
        center=(1280 / 2 - 75, 65))
    card_j2_2: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/card/card_back.png"))
    rect_card_j2_2: pygame.rect.Rect = card_j2_2.get_rect(
        center=(1280 / 2 + 75, 65))

    # setup the next player dealer token
    dealer_token_2: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/Dealer_Token.png"))
    rect_j2_token: pygame.rect.Rect = dealer_token_2.get_rect(
        center=(1280 / 2 + 225, 75))

    # setup the next player bankroll
    bankroll_j2: pygame.surface.Surface = pygame.image.load(
        resource_path("jeu/assets/images/chip.png"))
    rect_bankroll_j2: pygame.rect.Rect = bankroll_j2.get_rect(
        center=(1280 / 2 - 225, 75))
    ...
    table: pygame.surface.Surface = pygame.image.load(
    ...
    def reveal_flop() -> None:
        ...
        """
        list_board_card[0] = pygame.image.load(resource_path(
            f"jeu/assets/images/card/{dealer.board.cards[0].suit}/{dealer.board.cards[0].value}.png"))
        list_board_card[1] = pygame.image.load(resource_path(
            f"jeu/assets/images/card/{dealer.board.cards[1].suit}/{dealer.board.cards[1].value}.png"))
        list_board_card[2] = pygame.image.load(resource_path(
            f"jeu/assets/images/card/{dealer.board.cards[2].suit}/{dealer.board.cards[2].value}.png"))
        ...
        if bet_textbox.text != '':
            if bet_textbox.text != None and int(bet_textbox.text) <= list_players[current_player].bankroll.bankroll:
                if (int(bet_textbox.text) >= dealer.blind * 2):
                    ...
                    list_players[current_player].bankroll.bankroll -= int(
                        bet_textbox.text)
    ...
    def call_handler() -> None:
        ...
        blind: int = list_players[who_next_player(
        )].bet - list_players[current_player].bet
        ...
        # if player's check
            ...
            if step_one:
                ...
                winner: int = (dealer.compare_hand(
                    current_player, who_next_player()) + 1) % len(dealer.players)
                if winner == -1:
    ...
    while game_loop:
        ...
        screen.blit(players_token_rect[current_dealer]
                    [0], players_token_rect[current_dealer][1])
        ...
        if end_turn:
            ...
            if current_dealer != current_dealer:
                swap_player()
            dealer.blind = 0
            ...
            if list_players[who_next_dealer()].bankroll.bankroll >= big_blind // 2:
                ...
                list_players[who_next_dealer(
                )].bankroll.bankroll -= big_blind // 2
            else:
                dealer.total_blind += list_players[who_next_dealer()
                                                   ].bankroll.bankroll
        ...
        screen.blit(menu_font.get_font(25).render(str(list_players[current_player].bankroll.bankroll), True, "#EEEEEE"), menu_font.get_font(
            25).render(str(list_players[current_player].bankroll.bankroll), True, "#EEEEEE").get_rect(center=(1280 / 2 + 225, 720 - 75)))
        screen.blit(menu_font.get_font(25).render(str(list_players[who_next_player()].bankroll.bankroll), True, "#EEEEEE"), menu_font.get_font(
            25).render(str(list_players[who_next_player()].bankroll.bankroll), True, "#EEEEEE").get_rect(center=(1280 / 2 - 225, 75)))
        ...
        if end_turn:
            ...
            for player in dealer.players:
                ...
                if player.bankroll.bankroll <= 0:
                    ...
                    end_label: pygame.surface.Surface = menu_font.get_font(
                        75).render("Game Over", True, "white")
                    end_rect: pygame.rect.Rect = end_label.get_rect(center=(
                        end_popup.surface.get_size()[0]//2, end_popup.surface.get_size()[1]//2*0.8))
                    ...
                    end_popup_quit = Button(
                        ...
                        image=None,
                        position=(end_popup.surface.get_size()[
                                  0]//2, end_popup.surface.get_size()[1]//1.2),
                        text="QUIT",
